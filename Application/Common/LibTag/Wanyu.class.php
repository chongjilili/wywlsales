...
<?php
    ...
    \$prolist['hdpic'] = preg_replace('/!(.)+$/', '', \$prolist['litpic']);
...
str;
 ...
 // 
    //多条件筛选的产品列表
    public function _proattrlist($attr, $content)
    {
        $flag     = empty($attr['flag']) ? '' : $attr['flag'];
        $typeid   = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']);
        $arcid    = empty($attr['arcid']) ? '' : $attr['arcid'];
...
<?php
    ...
    \$_keyword = "$keyword";
    \$_arcid = "$arcid";
    \$_attrvalip = I('attrvalip') ;
    // p(\$_attrvalip);
    \$Attribute = new \Manage\Model\AttributeModel();
    \$productid_str  =  \$Attribute->getAttrRightid( \$_attrvalip);
    ...
    if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
        ...
        \$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
        //p(\$ids);
       // p(\$productid_str); //5,7,8,9 有可能没有匹配的id
        if(\$productid_str==''){
            \$where = array('product.status' => 0, 'product.cid'=> array('IN','1000000'),'product.id'=> array('IN','1000000') );
        }else{
            \$where = array('product.status' => 0, 'product.cid'=> array('IN',\$ids), 'product.id'=> array('IN',\$productid_str));
        }
...
<?php
    ...
    \$list['hdpic'] = preg_replace('/!(.)+$/', '', \$list['litpic']);
